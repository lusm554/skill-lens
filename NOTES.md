...
[2025 Dec 23 16:36] Для начала нужно определить тип ML‑задачи для таксономии ролей по вакансиям. Это задача классификации текста, причём open set, так как роли могут появляться и исчезать со временем. Например, такие роли, как LLM‑разметчик или Prompt Engineer, появились недавно. Следовательно, мы решаем задачу Retrieval‑Augmented Classification (RAC) с элементами metric learning для текста.
...
[2025 Dec 19 15:30] Забегая вперед, думаю, как лучше запустить always‑on демо проекта. Чтобы любой желающий в любое время мог зайти и «потрогать» проект. По сути, 24/7 нужны только сервис frontend+backend и БД/S3. БД/S3 можно вынести в облако, возможно, даже во free tier поместится. А вот сервис frontend+backend можно либо вынести на легкий хостинг (2–3 тыс. руб. в год + домен), из плюсов — сервис всегда доступен без задержки, либо в что-то вроде GCS Cloud Run: из плюсов — можно уместиться во free tier, из минусов — посетитель сайта будет ждать 5–10 секунд, пока запустится машина. Остается вопрос с «тяжелыми вычислениями». Допустим, вакансии грузятся 15–20 минут в bronze-слой, затем скорим модели, собираем агрегаты, загружаем в gold-слой — пусть это занимает не более 120 минут. В общем, нужно около 140 минут (2,5 часа) на «тяжелые вычисления». На этот период можно динамически аллоцировать контейнер по схеме pay‑as‑you‑go. В таком случае нужен либо оркестратор, либо расписание. Вопрос нужно еще доработать.
