...
[2025 Dec 19 15:30] Забегая вперед, думаю как лучше запустить always on демо проекта. Чтобы любой желающий в любое время мог зайти и "потрогать" проект. По сути 24/7 нужны только сервис frontend+backend и БД/S3. БД/S3 можно вынести в облако, возможно даже во free tier поместится. А вот сервис frontend+backend можно либо вынести в легкий хостинг (2-3тыс руб в год + домен), из плюсов сервис всегда доступен без задержке, либо вынести в что-то вроде GCS Cloud Run, из плюсов можно уместиться в free tier, из минусов посетитель сайта будет ждать 5-10 секунд пока запустится машина. Остается вопрос с "тяжелыми вычислениями". Допустим вакансии грузятся 15-20 мин в bronze layer, затем скорим модели, собиарем агрегаты, грузим в gold layer, ну будет это занимать не более 120 мин. В общем нужно 140мин/2.5 часа на "тяжелые вычисления". На этот период можно динамически алоцировать контейнер с pay-as-you-go. В таком случае нужен либо оркестратор, либо расписание. Вопрос нужно доработать еще.
