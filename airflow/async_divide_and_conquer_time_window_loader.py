...
async def rate_limited_get(session, url, params, headers, limiter, log, tries=MAX_RETRIES):
...
async def get_found(session, date_from, date_to, limiter, log):
    ...
    data, status = await rate_limited_get(session, API_URL, params, headers, limiter, log)
    if data:
...
async def fetch_vacancies_async(session, date_from, date_to, limiter, log):
    ...
    tasks = [fetch_page_limited(session, date_from, date_to, page, limiter, log) for page in range(total_pages)]
...
async def recursive_fetch_async(session, date_from, date_to, limiter, results_raw, log, failed_windows, overlap_minutes=5):
    ...
    if found > 2000:
        ...
        left_end = mid + overlap
        right_start = mid - overlap
        await recursive_fetch_async(session, date_from, left_end, limiter, results_raw, log, failed_windows, overlap_minutes)
...
async def main_async(input_start, input_end, outfile="vacancies_raw.jsonl", failed_windows_file="failed_windows.jsonl", max_concurrent=5):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        while curr_start < end:
            ...
            await recursive_fetch_async(session, curr_start, curr_end, limiter, results_raw, log, failed_windows)
            curr_start = curr_end
...
if __name__ == "__main__":
    ...
    asyncio.run(main_async(input_start, input_end, outfile="vacancies_raw.jsonl", failed_windows_file="failed_windows.jsonl", max_concurrent=MAX_CONCURRENT_REQUESTS))
